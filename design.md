...
아래 행위는 어떤 상황에서도 허용되지 않는다:

1. **전체 페이지를 한 번에 구현하지 않는다.** 반드시 섹션별로 나누어 구현한다.
2. **스크린샷만 보고 추측 구현하지 않는다.** 반드시 `get_figma_data`에서 읽은 디자인 데이터(텍스트, 색상, 간격, 폰트)를 기반으로 구현한다.
3. **단계를 건너뛰지 않는다.** 1→2→3→4→5→6→7 순서를 반드시 따른다. 장애가 발생하면 아래 "장애 대응"을 따른다.
4. **색상, 폰트, 간격을 하드코딩하지 않는다.** 반드시 토큰 CSS 변수를 사용한다.

---

## 장애 대응

워크플로우 진행 중 발생할 수 있는 장애와 대응 방법:

### `get_figma_data` 결과가 너무 클 때
1. depth 파라미터를 줄여서 **최상위 구조만** 먼저 조회한다.
2. 최상위 자식 노드들의 nodeId를 추출한다.
3. 각 자식 노드를 **개별로** `get_figma_data` 호출하여 섹션별 데이터를 확보한다.
4. 그래도 단일 섹션이 너무 크면, 해당 섹션의 하위 노드를 다시 분할한다.

### `download_figma_images` 실패 시 (0개 반환)
1. `get_screenshot`으로 해당 섹션의 스크린샷을 캡처한다.
2. 피그마에서 개별 에셋 노드의 nodeId를 찾아 `download_figma_images`를 **노드 단위로 재시도**한다.
3. 그래도 실패하면 사용자에게 보고하고 대안을 논의한다 (수동 다운로드 등).
4. **절대로 플레이스홀더 div로 대체한 뒤 넘어가지 않는다.**

### 토큰 CSS 변수가 `tokens.generated.css`에 없을 때
1. [2]단계 리포트에서 이미 식별되어야 한다.
2. `packages/foundation/`에 토큰을 추가 → `pnpm generate:tokens` 재빌드.
3. 재빌드 없이 하드코딩으로 우회하지 않는다.

### 아이콘 SVG가 `packages/icons/svg/`에 없을 때
1. [2]단계 리포트에서 이미 식별되어야 한다.
2. 아이콘 라이브러리에서 용도가 유사한 아이콘으로 대체한다.
3. 대체 시 사용자에게 어떤 아이콘으로 대체했는지 반드시 알린다.
